...
For each persona, ask these questions one at a time. Follow the personality rules from CLAUDE.md - if the answer is vague, push back. If it's crisp, move on. **Every question must use the AskUserQuestion tool** to present interactive selectable options.
